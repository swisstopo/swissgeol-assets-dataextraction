...
df = pd.DataFrame(
    output, columns=["asset_id", "sgs_id", "file_name", "file_page_count", "page_number", "width", "height", "area"]
)
...
print("{} unique assets and {} unique files affected.".format(df["asset_id"].nunique(), unique_file_count))
...
print("* {} files with only small pages ({} pages).".format(len(full_files), full_files["count"].sum()))

majority_files = grouped_max[
    grouped.size().between(0.5 * grouped_max["file_page_count"], grouped_max["file_page_count"], inclusive="left")
]
print("* {} files with 50%-99.9% small pages ({} pages).".format(len(majority_files), majority_files["count"].sum()))

minority_files = grouped_max[grouped.size() < 0.5 * grouped_max["file_page_count"]]
print("* {} files with 0.1%-49.9% small pages ({} pages).".format(len(minority_files), minority_files["count"].sum()))
