...
mismatch = combined[
    (abs(combined["height_new"] - combined["height_old"]) > 0.01)
    | (abs(combined["width_new"] - combined["width_old"]) > 0.01)
]
...
combined["language_change"] = (
    combined["language_code_old"].fillna("") + "->" + combined["language_code_new"].fillna("")
)
