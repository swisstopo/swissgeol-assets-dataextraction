...
def extract_geometric_lines(page: pymupdf.Page) -> list:
    ...
    merging_params = {
        "reference_size": 1000,
        "merging_tolerance": 8.0,  # maximum distance between two lines that are merged for a reference of width=1000
        "angle_threshold": 5.0,  # maximum difference in degrees between two line segments that are merged
        "min_line_length": 100,
...
def _are_parallel(line1: Line, line2: Line, angle_threshold: float) -> bool:
    ...
    """
    return (
        np.abs(line1.line_angle - line2.line_angle) < angle_threshold
        or 180 - np.abs(line1.line_angle - line2.line_angle) < angle_threshold
    )
