...
def main(config_path: str, out_directory: str, tuning: bool = False, parallel: bool = True, max_workers: int = None):
    ...
    load_fn = load_data_and_labels_parallel if parallel else load_data_and_labels_sequential

    start_time = time.time()
    if parallel:
        X_train, y_train = load_fn(train_folder, label_lookup, max_workers=max_workers)
        X_val, y_val = load_fn(val_folder, label_lookup, max_workers=max_workers)
    else:
